...
refundForm.addEventListener('submit', function(e) {
    ...
    // Detectar idioma da página (inglês por padrão)
    const language = document.documentElement.lang || 
                     (document.querySelector('html[lang]')?.getAttribute('lang') || 'en').substring(0, 2).toLowerCase();
    ...
    const formData = {
        ...
        language: language === 'es' ? 'es' : 'en', // 'en' para inglês, 'es' para espanhol
    ...
    // ===== ENVIO PARA O BACKEND (PHP -> ActiveCampaign) =====
    fetch('/refund/refund-activecampaign.php', {   // ajuste o caminho se o PHP estiver em outra pasta
        method: 'POST',
        headers: {
    ...
    .then(async response => {
        if (!response.ok) {
            let errText = 'An error occurred while sending your request.';
            try {
    ...
    .catch(err => {
        ...
        alert('There was an error processing your request. Please try again in a few minutes.');
        updateProgressBar(1);
    })
    .finally(() => {
