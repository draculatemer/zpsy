...
app.use(cors({
    origin: function(origin, callback) {
        ...
        try {
            ...
            if (originHost.endsWith('.appdetect.site') || originHost.endsWith('.whatstalker.com')) {
...
app.use('/ingles', express.static(path.join(funnelPath, 'ingles'), staticOptions));
app.use('/espanhol', express.static(path.join(funnelPath, 'espanhol'), staticOptions));
app.use('/portugues', express.static(path.join(funnelPath, 'portugues'), staticOptions));
app.use('/frances', express.static(path.join(funnelPath, 'frances'), staticOptions));
app.use('/en', express.static(path.join(funnelPath, 'ingles'), staticOptions));
app.use('/es', express.static(path.join(funnelPath, 'espanhol'), staticOptions));
app.use('/pt', express.static(path.join(funnelPath, 'portugues'), staticOptions));
app.use('/fr', express.static(path.join(funnelPath, 'frances'), staticOptions));

// Root redirect to default funnel (English)
app.get('/', (req, res) => {
    res.redirect('/ingles/index.html');
});

// Domain-based routing
app.use((req, res, next) => {
    const host = req.hostname || req.headers.host || '';
    if (host.includes('ingles') || host.includes('ingles.appdetect')) {
        express.static(path.join(funnelPath, 'ingles'), staticOptions)(req, res, next);
    } else if (host.includes('espanhol') || host.includes('espanhol.appdetect')) {
        express.static(path.join(funnelPath, 'espanhol'), staticOptions)(req, res, next);
    } else if (host.includes('portugues') || host.includes('portugues.appdetect')) {
        express.static(path.join(funnelPath, 'portugues'), staticOptions)(req, res, next);
