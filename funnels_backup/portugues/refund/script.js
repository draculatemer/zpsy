...
function initCountrySelector() {
    ...
    function renderCountries(filter = '') {
        ...
        if (filtered.length === 0) {
            countryList.innerHTML = '<div class="no-results">País não encontrado</div>';
            return;
        }
...
function initCharCounter() {
    ...
    details.addEventListener('input', function() {
        ...
        if (len >= MIN_FEEDBACK_CHARS) {
            ...
            charStatus.textContent = '⚠ Mais detalhes necessários';
...
function validateStep(step) {
    ...
    if (step === 3) {
        ...
        if (len < MIN_FEEDBACK_CHARS) {
            showError('details', `Por favor, forneça pelo menos ${MIN_FEEDBACK_CHARS} caracteres. Você tem ${len} caracteres.`);
            isValid = false;
...
async function submitForm() {
    ...
    try {
        ...
        const waMessage = encodeURIComponent(
            `Olá, sou ${formData.fullName} e quero confirmar minha solicitação de reembolso do Whats Spy (Protocolo: ${protocol}). Motivo: ${reasonText}.`
        );
        ...
        showToast('Erro ao enviar a solicitação. Por favor, tente novamente.', 'error');
    } finally {
